...
//-----------------------------Random Function-----------------------------
#define RAND 62     // "111110"; -- RAND Rx     -- Rx <- M[regRAND]	Format: < inst(6) | Rx(3) |xxx| xxxx >
...
int main()
{
	...
	int PC=0, IR=0, SP=0, MAR=0, rx=0, ry=0, rz=0, regRAND=0, IncRAND=0, COND=0, RW=0, DATA_OUT=0;     //-----------adicionado o RegRAND
	int LoadPC=0, IncPC=0, LoadIR=0, LoadSP=0, IncSP=0, DecSP=0, LoadMAR=0, LoadFR=0;
	int M0=0, M1=0, M2=0, M3=0, M4=0, M5=0, M6=0;                                           //-----------adicionado o M0
	int selM0=0, selM1=0, selM2=0, selM3=0, selM4=0, selM5=0, selM6=0;                      //-----------adicionado o selM0
	int LoadReg[8] = {0};
	...
	int TECLADO;
/*
	MEMORY[TAMANHO_MEMORIA - 30] = 2;
	MEMORY[TAMANHO_MEMORIA - 29] = 0;
	MEMORY[TAMANHO_MEMORIA - 28] = 7;
	MEMORY[TAMANHO_MEMORIA - 27] = 1;
	MEMORY[TAMANHO_MEMORIA - 26] = 6;
	MEMORY[TAMANHO_MEMORIA - 25] = 3;
	MEMORY[TAMANHO_MEMORIA - 24] = 2;
	MEMORY[TAMANHO_MEMORIA - 23] = 4;
	MEMORY[TAMANHO_MEMORIA - 22] = 0;
	MEMORY[TAMANHO_MEMORIA - 21] = 4;
	MEMORY[TAMANHO_MEMORIA - 20] = 3;
	MEMORY[TAMANHO_MEMORIA - 19] = 1;
	MEMORY[TAMANHO_MEMORIA - 18] = 7;
	MEMORY[TAMANHO_MEMORIA - 17] = 3;
	MEMORY[TAMANHO_MEMORIA - 16] = 0;
	MEMORY[TAMANHO_MEMORIA - 15] = 0;
	MEMORY[TAMANHO_MEMORIA - 14] = 5;
	MEMORY[TAMANHO_MEMORIA - 13] = 2;
	MEMORY[TAMANHO_MEMORIA - 12] = 5;
	MEMORY[TAMANHO_MEMORIA - 11] = 4;
	MEMORY[TAMANHO_MEMORIA - 10] = 1;
	MEMORY[TAMANHO_MEMORIA -  9] = 7;
	MEMORY[TAMANHO_MEMORIA -  8] = 6;
	MEMORY[TAMANHO_MEMORIA -  7] = 2;
	MEMORY[TAMANHO_MEMORIA -  6] = 5;
	MEMORY[TAMANHO_MEMORIA -  5] = 4;
	MEMORY[TAMANHO_MEMORIA -  4] = 3;
	MEMORY[TAMANHO_MEMORIA -  3] = 6;
	MEMORY[TAMANHO_MEMORIA -  2] = 1;
	MEMORY[TAMANHO_MEMORIA -  1] = 5;
*/
...
loop:
	...
	if (RW == 1) MEMORY[M0] = M5; // de M1 para M0
	...
	switch(state)
	{
		case STATE_RESET:
			...
			IR = 0;
			MAR = 0;
			SP = TAMANHO_MEMORIA -1 -30; //--------------------------------------INICIA O PONTEIRO DA PILHA 30 POSICOES APOS O FINAL DA MEMORIA
		...
		case STATE_DECODE:
			...
			switch(opcode){
                ...
                case RAND:
                    selM0 = sRAND; //1000000000000000
                    if (pega_pedaco(regRAND, 15, 15) == 1) {
                        regRAND = TAMANHO_MEMORIA - 30;printf("\nRESET in regRAND\n") ;}
                    RW = 0;
                    selM2 = sDATA_OUT;
                    LoadReg[rx] = 1;
		    //IncPC = 1;
...
// Selecao do Mux0
	if       (selM0 == sM1)   M0 = M1;
	else if  (selM0 == sRAND) M0 = regRAND;       //------------------Somador soma o valor do regRAND com o incremento

	if(M0 > (TAMANHO_MEMORIA)) {
		M0 = 0;
	...
	if (RW == 0) DATA_OUT = MEMORY[M0];  // Tem que vir antes do M2 que usa DATA_OUT
